...
void  OSInit (OS_ERR  *p_err)
{
    ...
    OS_PrioInit(); //就绪优先级位映射表初始化                                         /* Initialize the priority bitmap table                   */

    OS_RdyListInit(); //就绪任务列表初始化                                         /* Initialize the Ready List                              */
...
void  OSIntEnter (void)
{
    ...
    OSIntNestingCtr++;//记录中断嵌套的次数                 /* Increment ISR nesting level                            */
}
...
void  OSIntExit (void)
{
    ...
    CPU_INT_DIS();//关闭全局中断
    //判断OSIntNestingCtr是否是0，若是说明之前没有发生中断，跳出
    ...
    //判断优先级最高的任务是否是当前运行的任务。以避免不必要的调度。若是，直接return。
...
//先进行检查，确保其不是从中断服务程序中被调用。因为OSSched (void)是任务级调度器
    if (OSIntNestingCtr > (OS_NESTING_CTR)0) {              /* ISRs still nested?                                     */
        return;                                             /* Yes ... only schedule when no nested ISRs              */
    }
//确认调度器有没有上锁，若果上了锁，说明用户并不想启动调度器，这时OSSched (void)直接返回 。
    if (OSSchedLockNestingCtr > (OS_NESTING_CTR)0) {        /* Scheduler locked?                                      */
        return;                                             /* Yes                                                    */
    }

    CPU_INT_DIS();//关闭中断，防止被中断打断
    OSPrioHighRdy   = OS_PrioGetHighest();  //找到就绪了的任务的最高优先级   /* Find the highest priority ready                        */
    OSTCBHighRdyPtr = OSRdyList[OSPrioHighRdy].HeadPtr;//获取这个优先级下任务的控制块

    //判断优先级最高的任务是否是当前运行的任务。以避免不必要的调度。若是，直接return。
    //OSTCBCurPtr：指向当前任务的控制块OS_TCB。OSTCBHighRdyPtr指向将要切换的任务的控制块。
    if (OSTCBHighRdyPtr == OSTCBCurPtr) {                   /* Current task is still highest priority task?           */
        CPU_INT_EN();  //打开中断                                     /* Yes ... no need to context switch                      */
        return;
    }
    ...
    OS_TASK_SW();//任务切换  将挂起的任务的寄存器压入堆栈，然后将高优先级的寄存器从栈中恢复到CPU的寄存器中。                                           
    CPU_INT_EN();//打开中断 ，起到临界保护的作用。
}
...
void  OSStart (OS_ERR  *p_err)
{
    ...
    if (OSRunning == OS_STATE_OS_STOPPED) {  //参数检验，多任务尚未启动
