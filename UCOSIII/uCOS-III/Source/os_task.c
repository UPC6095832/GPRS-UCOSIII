                    ...
                    OS_ERR        *p_err)
{
    ...
    //
    OS_TaskInitTCB(p_tcb);//初始化控制块                                  /* Initialize the TCB to default values                   */
   ...
   *p_err = OS_ERR_NONE;
    ...
    /*
    1.在创建一个新任务时，必须把系统启动这个任务时所需的CPU各个寄存器初始值存放在任务堆栈中，
    这样当任务获取CPU的使用权时，就把任务堆栈的内容复制到CPU各个寄存器，从而可以顺利地启动并运行
    2.初始化任务堆栈，寄存器都保存在堆栈中
    */
    ...
    /******************初始化控制块各个参数******************/                                                  
    p_tcb->TaskEntryAddr = p_task; //任务函数地址                        
    p_tcb->TaskEntryArg  = p_arg;  //传递给任务函数的参数                        

    p_tcb->NamePtr       = p_name; //任务名字 

    p_tcb->Prio          = prio; //任务优先级                         

    p_tcb->StkPtr        = p_sp;                            
    p_tcb->StkLimitPtr   = p_stk_limit; //任务堆栈深度限位                    

    p_tcb->TimeQuanta    = time_quanta; //当使能时间片轮转时的时间片长度                 
...
#endif
    p_tcb->ExtPtr        = p_ext;//用户补充的存储区                     
    p_tcb->StkBasePtr    = p_stk_base;//任务堆栈基地址                   
    p_tcb->StkSize       = stk_size; //任务堆栈大小                      
    p_tcb->Opt           = opt;  //任务选项                            
